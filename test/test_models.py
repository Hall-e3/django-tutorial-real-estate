...
def test_user_str(base_user):
    ...
    username = f"{base_user.username}"
    assert base_user.__str__() == username
...
def test_create_superuser_with_no_email(user_factory):
    """ "Test creating a super user without an email raises an error"""
    with pytest.raises(ValueError) as err:
...
def test_create_superuser_with_no_password(user_factory):
    """ "Test creating a super user without an password raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(password=None, is_superuser=True, is_staff=True)
    assert str(err.value) == "Admin Account: super admin user must have a password"


def test_user_email_incorrect(user_factory):
    """Test creating a new user with incorrect email raises an error"""
    with pytest.raises(ValueError) as err:
        user_factory.create(email="realeststate.com")
